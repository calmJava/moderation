...
module.exports = {
    ...
    run: async(client, message, args) => {
        let embed = new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('2F3136').setTimestamp().setThumbnail(message.author.avatarURL).setFooter('Rash Was Here!');
        ...
        message.channel.send(embed.setDescription("Kullanıcının ses kanallarındaki susturulması açıldı!")).then(x => x.delete({ timeout: 5000 })).then(message.react(client.config.tik));
        channel.send(embed.setDescription(`${message.author} adlı yetkili tarafından ${member} adlı kullanıcının sesteki susturulması kaldırıldı!`))
    }
}
