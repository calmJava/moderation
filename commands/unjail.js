...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let embed = new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp().setThumbnail(message.author.avatarURL).setColor('2F3136').setFooter('Rash Was Here');
        let member = message.mentions.members.first();
        if (!member) return message.channel.send(embed.setDescription("Kullanıcı bulunamadı veya etiketlenmedi! \n `.unjail @Lucy/ID`")).then(x => x.delete({ timeout: 5000 })).then(message.react(client.config.no));
